...
**Domain:** Amazon KDP Book Publishing Web App
...
TipTap Content (HTML/JSON)
  → Transform to @react-pdf/renderer components
  → Render to PDF buffer (server-side via renderToStream)
  → pdf-lib merges cover image + sets metadata
  → Final KDP-ready PDF
```
...
```bash
# Core framework
pnpm create next-app@15.5 publik --typescript --tailwind --eslint --app --src-dir

# Rich text editor
pnpm add @tiptap/react @tiptap/starter-kit @tiptap/extension-table-of-contents @tiptap/pm

# PDF generation
pnpm add @react-pdf/renderer pdf-lib @pdf-lib/fontkit
...
pnpm add zod lucide-react sonner nuqs clsx tailwind-merge date-fns
...
**If starting as personal tool (single user):**
...
**If targeting SaaS from the start:**
...
**If KDP formatting requirements prove complex:**
...
---
*Stack research for: Amazon KDP Book Publishing Web App*
*Researched: 2026-03-30*
